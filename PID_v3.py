...
with st.sidebar.expander("🖥️ Вид (мнемосхема / тренд)", expanded=False):
    mimic_scale = st.slider("Масштаб мнемосхемы, %", 40, 100, 80, 5)
    chart_height = st.slider("Высота тренда, px", 250, 800, 380, 20)
    window_pct = st.slider("Окно тренда (зум по времени), %", 5, 100, 100, 5)
...
def build_mimic_svg(ss, mimic_w):
    ...
    <svg viewBox="0 0 820 430" width="{mimic_w}%" preserveAspectRatio="xMidYMid meet"
         style="background:#0e1621;border-radius:10px;max-width:100%;height:auto;">
      <!-- Заголовок -->
      ...
      <!-- Труба нагнетания + датчик давления -->
...
# высота iframe пропорциональна масштабу; base ~ ширина 900px * (430/820)
iframe_h = int(900 * (mimic_scale / 100.0) * (430 / 820)) + 40
components.html(build_mimic_svg(ss, mimic_scale), height=iframe_h, scrolling=False)
...
def windowed(df):
    """Возвращает последние window_pct% точек — зум по времени."""
    n = len(df)
    keep = max(2, int(n * window_pct / 100.0))
    return df.iloc[-keep:]


if ss.history and selected:
    df = pd.DataFrame(ss.history).set_index("t")
    st.line_chart(windowed(df)[selected], height=chart_height)
elif not selected:
...
if ss.history:
    ...
    with colA:
        ...
        st.line_chart(dfw[["Расход, м³/ч"]], height=chart_height)
    with colB:
        st.subheader("📊 Давление, бар")
        st.line_chart(dfw[["Давление, бар"]], height=chart_height)
